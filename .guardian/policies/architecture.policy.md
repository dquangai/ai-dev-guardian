...
`checks/` (áp policy lên diff) → `orchestrator.ts` / `cli.ts` (điều phối, entrypoint). Một layer chỉ
được phụ thuộc vào layer cùng cấp hoặc thấp hơn, không bao giờ phụ thuộc ngược lên layer cao hơn nó.
Các cặp import bị cấm tường minh được kiểm tra tự động qua `rules` ở trên (`architectureRulesCheck.ts`).
