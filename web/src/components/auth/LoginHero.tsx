...
const ACTIVE_LOGS = [
  { time: '10:06:50', action: 'policy.check: secret-scan', status: 'PASSED', color: 'text-emerald-400' },
  { time: '10:07:12', action: 'policy.check: dependency-rules', status: 'PASSED', color: 'text-emerald-400' },
  { time: '10:08:03', action: 'policy.check: architecture-rules', status: 'PASSED', color: 'text-emerald-400' },
  { time: '10:09:16', action: 'policy.check: llm-policy-check', status: 'BLOCKED', color: 'text-amber-400 font-semibold animate-pulse' },
]
...
export function LoginHero() {
  return (
    <div className="relative hidden overflow-hidden bg-gradient-to-br from-[#80070B] via-[#9E0B10] to-[#590407] px-12 py-16 text-white lg:flex lg:w-1/2 lg:flex-col lg:justify-between border-r border-[#80070B]/35">
      {/* Decorative Gold & White Sparkles / Ambient Light */}
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-gradient-to-b from-white/5 to-transparent rounded-full blur-[100px] pointer-events-none" />
      <div className="absolute bottom-[-10%] left-[-10%] w-[350px] h-[350px] bg-gradient-to-tr from-amber-500/10 to-transparent rounded-full blur-[80px] pointer-events-none" />
      ...
      <div className="relative z-10 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="block text-[8px] font-medium tracking-[0.1em] text-amber-400/80 uppercase">AI Security Division</span>
          </div>
        </div>
      </div>
      ...
      <div className="relative z-10 my-auto flex flex-col gap-10">
        <div className="space-y-3">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-500/10 border border-amber-500/20 px-3 py-1 text-[10px] font-semibold tracking-wider text-amber-400 uppercase">
            <Sparkles size={10} />
            Enterprise Edition
          </span>
        ...
        <div className="rounded-xl border border-white/15 bg-black/25 backdrop-blur-md p-6 shadow-2xl">
          ...
          <div className="mt-5 rounded-lg bg-[#590407]/40 border border-white/10 p-3 flex items-start gap-3">
            <Shield size={16} className="text-amber-400 mt-0.5 shrink-0" />
            <div className="text-[11px] text-white/95 leading-relaxed">
              <strong className="text-amber-400">Cảnh báo đẩy:</strong> Phát hiện vi phạm quy tắc kiến trúc trong file <code className="bg-black/20 px-1 py-0.5 rounded text-white font-mono">rbac.ts</code>. Đã chặn commit thành công.
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 border-t border-white/10 pt-6">
          ...
          <div>
            <div className="text-xl font-bold text-amber-400">Secured</div>
